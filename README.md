...
Ideal steps with Trivy:

- `trivy fs --format cyclonedx --output result.json ./dcs-pylot-dash-ui/pnpm-lock.yaml`

Non-ideal steps with `npm` only:

- generate `package-lock.json`: `npm i --package-lock-only`
- generate SBOM: `npm sbom --package-lock-only=true --sbom-type=application --sbom-format=cyclonedx`
