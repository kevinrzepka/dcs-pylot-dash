...
local previousFunctionDefinitions = {}
previousFunctionDefinitions.LuaExportStart = LuaExportStart
previousFunctionDefinitions.LuaExportAfterNextFrame = LuaExportAfterNextFrame
previousFunctionDefinitions.LuaExportStop = LuaExportStop
...
local function debug_lua_environment()
    log.write("DCSPylotDash", log.INFO, "=== Lua Environment Debug ===")
    log.write("DCSPylotDash", log.INFO, "Lua version:" .. _VERSION)
    log.write("DCSPylotDash", log.INFO, "Package path:" .. package.path)
    log.write("DCSPylotDash", log.INFO, "Package cpath:" .. package.cpath)
end
...
function LuaExportStart()
    ...
    if previousFunctionDefinitions.LuaExportStart ~= nil then
        previousFunctionDefinitions.LuaExportStart()
    end
end
...
function generate_json_data()
    ...
    data.ias = {}
    data.ias.kts = (airspeed or 0) * 1.9438400000000629
    data.mach = (mach or 0)
    data.tas = {}
    data.tas.kts = (tas or 0) * 1.9438400000000629
    data.heading = {}
    data.heading.degrees = (heading or 0) * 57.29577951308232
    data.altitude = {}
    data.altitude.msl = {}
    data.altitude.msl.ft = (altitude_msl or 0) * 3.28084
    data.altitude.agl = {}
    data.altitude.agl.ft = (altitude_agl or 0) * 3.28084
    data.fuel = {}
    data.fuel.internal = {}
    data.fuel.internal.lbs = (engine_info.fuel_internal or 0) * 12000.0
    data.arms = {}
    data.arms.gun_rounds = (payload_info.Cannon.shells or 0)
...
function LuaExportAfterNextFrame()
    ...
    if previousFunctionDefinitions.LuaExportAfterNextFrame ~= nil then
        previousFunctionDefinitions.LuaExportAfterNextFrame()
    end
end
...
function LuaExportStop()
    ...
    if previousFunctionDefinitions.LuaExportStop ~= nil then
        previousFunctionDefinitions.LuaExportStop()
    end
end
